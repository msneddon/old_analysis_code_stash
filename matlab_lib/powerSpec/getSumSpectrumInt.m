function [ power, freq, segCount ] = getSumSpectrumInt( series, dt, intervalLength )
%GETAVGSPECTRUM Takes a time series and generates the sumation of the 
%   power spectrum based on the given dt.  Notice that this does not
%   produce an average power spectrum.  This allows you to run this
%   function on multiple series of differing lengths and average them all
%   together in the end.
%
...
while endIndex<=length(series)
    ...
    [segpower,freq] = getspec(series(startIndex:endIndex),dt);
